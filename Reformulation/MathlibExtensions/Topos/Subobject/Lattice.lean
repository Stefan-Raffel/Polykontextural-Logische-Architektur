...
Mathlib's `CompleteLattice (Subobject B)` (instance `CategoryTheory.Subobject.instCompleteLattice`,
anonymous in Mathlib, name generated by the elaborator and subject to change) requires
six hypotheses:
...
-- CompleteLattice (Subobject B) instance (cf. the `attribute [local instance]` line for
-- `CategoryTheory.Limits.has_smallest_coproducts_of_hasCoproducts` in
