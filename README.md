...
| Kennzahl | Wert |
|---|---:|
| geprueft (AxiomGate) | 3218 Konstanten |
| Axiom-Wachen | 452 ueber 50 Dateien |
| Saetze gesamt | 780 |
| Build-Jobs | 1306 |
| ausgewiesene Luecken | 0 (Whitelist leer) |

Kennzahlen gezaehlt am gruenen Build, Stand Commit `e97fdbe`.
...
Widerspruch, sondern der Bereich: die 780 laufen ueber `Reformulation/` allein, die Tafel
...
Zusaetzlich frieren 452 `#guard_msgs`-Wachen die gemessenen Axiom-Profile ein: aendert
ein Satz sein Profil, bricht der Bau. `Classical.choice` ist auf wenige Dateien begrenzt
und dort ausgewiesen.

Zu lesen mit einer Einschraenkung: von den 452 geschriebenen Wachen erzwingt `lake build`
**442** (in 49 Dateien). Die uebrigen 10 stehen in `Foreign/PeresMermin.lean`, das ueber
...
452 ist die Zahl der geschriebenen Wachen - Route `grep -rE '#guard_msgs.*in #print axioms'`
ueber `Reformulation/` und `Foreign/` -, 442 die der erzwungenen: dieselbe Route,
...
`.lake/` umfasst rund 7,5 GB und ist ausgeschlossen; der Quellbestand sind ~165 Dateien
unter 1 MB.
