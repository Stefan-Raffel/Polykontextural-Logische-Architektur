...
| Kennzahl | Wert |
|---|---:|
| geprueft (AxiomGate) | 3137 Konstanten |
| Axiom-Wachen | 422 ueber 47 Dateien |
| Saetze gesamt | 755 |
| Build-Jobs | 1303 |
...
|---|---:|---:|---|
| `Proemial/` | 59 | 446 | Proemialrelation, Transjunktion, Klon-Schranken |
...
Widerspruch, sondern der Bereich: die 755 laufen ueber `Reformulation/` allein, die Tafel
...
Zusaetzlich frieren 422 `#guard_msgs`-Wachen die gemessenen Axiom-Profile ein: aendert
ein Satz sein Profil, bricht der Bau. `Classical.choice` ist auf wenige Dateien begrenzt
und dort ausgewiesen.

Zu lesen mit einer Einschraenkung: von den 422 geschriebenen Wachen erzwingt `lake build`
**412** (in 46 Dateien). Die uebrigen 10 stehen in `Foreign/PeresMermin.lean`, das ueber
...
422 ist die Zahl der geschriebenen Wachen - Route `grep -rE '#guard_msgs.*in #print axioms'`
ueber `Reformulation/` und `Foreign/` -, 412 die der erzwungenen: dieselbe Route,
...
`.lake/` umfasst rund 7,5 GB und ist ausgeschlossen; der Quellbestand sind ~162 Dateien
unter 1 MB.
