...
|  |  |  |
| **BESTAND** |  |  |
| Module (.lean, verfolgt) | 192 | git ls-files '*.lean' — schliesst die Wurzeldatei Reformulation.lean ein |
| Saetze gesamt | 996 | geweitete grep-Satzroute ueber Reformulation/ allein (CLAUDE.md §3) |
| Saetze, verschaerfte Route | 996 | Gegenprobe: nach dem Namen muss ( { [ ⦃ : oder Zeilenende folgen |
| def-Deklarationen | 422 | geweitete def-Route ueber Reformulation/ |
| Statement-Pins | 107 | grep '^-- STATEMENT-PIN' (Prosa-Kriterien sind eine Zeitbombe, §3) |
|  |  |  |
| **WACHEN** |  |  |
| Wachen geschrieben | 718 | grep '#guard_msgs.*in #print axioms' ueber Reformulation/ UND Foreign/ |
| davon Dateien | 97 | dieselbe Route, -l |
| nackte #print axioms | 11 | gedruckt ist nicht gewacht (§8 Fallstrick 16); Lint-Gruppe (D) bricht darauf |
|  |  |  |
| **IMPORT-HUELLEN** |  |  |
| Aggregat | 138 | Huelle der Wurzel Reformulation.lean |
...
| Gate-Huelle | 139 | Huelle von Reformulation/AxiomGate.lean |
| Saetze im Aggregat | 815 | Satzroute, auf die Aggregathuelle eingeschraenkt |
| Wachen erzwungen | 708 | Wachenroute, auf die Huelle der Default-Targets eingeschraenkt |
...
| Gleichung *Partition* | ✓ | 192 gegen 192 |
| Gleichung *Gate=Aggregat+1* | ✓ | 139 gegen 139 |
| Gleichung *Wachen* | ✓ | 718 gegen 718 |
...
|  |  |  |
| **BAU (lake build)** |  |  |
| Build-Jobs | 1340 | lake build ueber die Default-Targets |
| geprueft (AxiomGate) | 3897 | Konstanten aus dem Importbaum, namensgefiltert auf Reformulation.* |
|  |  |  |
| **DOC-LINT** |  |  |
| (A.1) laufender Bestand | 75 | Superlativ, meldend |
