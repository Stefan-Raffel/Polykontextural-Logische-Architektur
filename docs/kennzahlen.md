...
|  |  |  |
| **BESTAND** |  |  |
| Module (.lean, verfolgt) | 195 | git ls-files '*.lean' — schliesst die Wurzeldatei Reformulation.lean ein |
| Saetze gesamt | 1058 | geweitete grep-Satzroute ueber Reformulation/ allein (CLAUDE.md §3) |
| Saetze, verschaerfte Route | 1058 | Gegenprobe: nach dem Namen muss ( { [ ⦃ : oder Zeilenende folgen |
| def-Deklarationen | 441 | geweitete def-Route ueber Reformulation/ |
| Statement-Pins | 109 | grep '^-- STATEMENT-PIN' (Prosa-Kriterien sind eine Zeitbombe, §3) |
|  |  |  |
| **WACHEN** |  |  |
| Wachen geschrieben | 780 | grep '#guard_msgs.*in #print axioms' ueber Reformulation/ UND Foreign/ |
| davon Dateien | 100 | dieselbe Route, -l |
| nackte #print axioms | 11 | gedruckt ist nicht gewacht (§8 Fallstrick 16); Lint-Gruppe (D) bricht darauf |
|  |  |  |
| **IMPORT-HUELLEN** |  |  |
| Aggregat | 141 | Huelle der Wurzel Reformulation.lean |
...
| Gate-Huelle | 142 | Huelle von Reformulation/AxiomGate.lean |
| Saetze im Aggregat | 877 | Satzroute, auf die Aggregathuelle eingeschraenkt |
| Wachen erzwungen | 770 | Wachenroute, auf die Huelle der Default-Targets eingeschraenkt |
...
| Gleichung *Partition* | ✓ | 195 gegen 195 |
| Gleichung *Gate=Aggregat+1* | ✓ | 142 gegen 142 |
| Gleichung *Wachen* | ✓ | 780 gegen 780 |
...
|  |  |  |
| **BAU (lake build)** |  |  |
| Build-Jobs | 1343 | lake build ueber die Default-Targets |
| geprueft (AxiomGate) | 4026 | Konstanten aus dem Importbaum, namensgefiltert auf Reformulation.* |
|  |  |  |
| **DOC-LINT** |  |  |
| (A.1) laufender Bestand | 80 | Superlativ, meldend |
