...
|  |  |  |
| **BESTAND** |  |  |
| Module (.lean, verfolgt) | 204 | git ls-files '*.lean' — schliesst die Wurzeldatei Reformulation.lean ein |
| Saetze gesamt | 1271 | geweitete grep-Satzroute ueber Reformulation/ allein (CLAUDE.md §3) |
| Saetze, verschaerfte Route | 1271 | Gegenprobe: nach dem Namen muss ( { [ ⦃ : oder Zeilenende folgen |
| def-Deklarationen | 490 | geweitete def-Route ueber Reformulation/ |
| Statement-Pins | 109 | grep '^-- STATEMENT-PIN' (Prosa-Kriterien sind eine Zeitbombe, §3) |
|  |  |  |
| **WACHEN** |  |  |
| Wachen geschrieben | 993 | grep '#guard_msgs.*in #print axioms' ueber Reformulation/ UND Foreign/ |
| davon Dateien | 108 | dieselbe Route, -l |
| nackte #print axioms | 11 | gedruckt ist nicht gewacht (§8 Fallstrick 16); Lint-Gruppe (D) bricht darauf |
|  |  |  |
| **IMPORT-HUELLEN** |  |  |
| Aggregat | 149 | Huelle der Wurzel Reformulation.lean |
...
| Gate-Huelle | 150 | Huelle von Reformulation/AxiomGate.lean |
| Saetze im Aggregat | 1090 | Satzroute, auf die Aggregathuelle eingeschraenkt |
| Wachen erzwungen | 983 | Wachenroute, auf die Huelle der Default-Targets eingeschraenkt |
...
| Gleichung *Partition* | ✓ | 204 gegen 204 |
| Gleichung *Gate=Aggregat+1* | ✓ | 150 gegen 150 |
| Gleichung *Wachen* | ✓ | 993 gegen 993 |
...
| geprueft (AxiomGate) | 4855 | Konstanten aus dem Importbaum, namensgefiltert auf Reformulation.* |
|  |  |  |
| **DOC-LINT** |  |  |
