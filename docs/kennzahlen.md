...
Stand: Commit `e696870` (1 geaenderte Dateien — Werte gelten NICHT am Commit).
...
|  |  |  |
| **BESTAND** |  |  |
| Module (.lean, verfolgt) | 171 | git ls-files '*.lean' — schliesst die Wurzeldatei Reformulation.lean ein |
| Saetze gesamt | 821 | geweitete grep-Satzroute ueber Reformulation/ allein (CLAUDE.md §3) |
| Saetze, verschaerfte Route | 821 | Gegenprobe: nach dem Namen muss ( { [ ⦃ : oder Zeilenende folgen |
| def-Deklarationen | 346 | geweitete def-Route ueber Reformulation/ |
| Statement-Pins | 65 | grep '^-- STATEMENT-PIN' (Prosa-Kriterien sind eine Zeitbombe, §3) |
|  |  |  |
| **WACHEN** |  |  |
| Wachen geschrieben | 568 | grep '#guard_msgs.*in #print axioms' ueber Reformulation/ UND Foreign/ |
| davon Dateien | 76 | dieselbe Route, -l |
| nackte #print axioms | 11 | gedruckt ist nicht gewacht (§8 Fallstrick 16); Lint-Gruppe (D) bricht darauf |
|  |  |  |
| **IMPORT-HUELLEN** |  |  |
| Aggregat | 116 | Huelle der Wurzel Reformulation.lean |
...
| Gate-Huelle | 117 | Huelle von Reformulation/AxiomGate.lean |
| Saetze im Aggregat | 637 | Satzroute, auf die Aggregathuelle eingeschraenkt |
| Wachen erzwungen | 558 | Wachenroute, auf die Huelle der Default-Targets eingeschraenkt |
...
| Gleichung *Partition* | ✓ | 171 gegen 171 |
| Gleichung *Gate=Aggregat+1* | ✓ | 117 gegen 117 |
| Gleichung *Wachen* | ✓ | 568 gegen 568 |
| Gleichung *Satzroute* | ✓ | ok gegen ok |
|  |  |  |
| **LUECKEN (selbstzaehlend — Prosa zaehlt mit, mit Absicht)** |  |  |
| N1 roh | 144 | WORTvorkommen (\bsorry\b) ueber den verfolgten Bestand; zaehlt die eigene Dokumentation mit |
...
|  |  |  |
| **BAU (lake build)** |  |  |
| Build-Jobs | 1311 | lake build ueber die Default-Targets |
