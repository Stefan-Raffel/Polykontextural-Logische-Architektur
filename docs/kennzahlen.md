...
Stand: Commit `2b8cfa6` (1 geaenderte Dateien — Werte gelten NICHT am Commit).
...
|  |  |  |
| **BESTAND** |  |  |
| Module (.lean, verfolgt) | 173 | git ls-files '*.lean' — schliesst die Wurzeldatei Reformulation.lean ein |
| Saetze gesamt | 833 | geweitete grep-Satzroute ueber Reformulation/ allein (CLAUDE.md §3) |
| Saetze, verschaerfte Route | 833 | Gegenprobe: nach dem Namen muss ( { [ ⦃ : oder Zeilenende folgen |
| def-Deklarationen | 349 | geweitete def-Route ueber Reformulation/ |
| Statement-Pins | 72 | grep '^-- STATEMENT-PIN' (Prosa-Kriterien sind eine Zeitbombe, §3) |
|  |  |  |
| **WACHEN** |  |  |
| Wachen geschrieben | 576 | grep '#guard_msgs.*in #print axioms' ueber Reformulation/ UND Foreign/ |
| davon Dateien | 78 | dieselbe Route, -l |
| nackte #print axioms | 11 | gedruckt ist nicht gewacht (§8 Fallstrick 16); Lint-Gruppe (D) bricht darauf |
|  |  |  |
| **IMPORT-HUELLEN** |  |  |
| Aggregat | 118 | Huelle der Wurzel Reformulation.lean |
...
| Gate-Huelle | 119 | Huelle von Reformulation/AxiomGate.lean |
| Saetze im Aggregat | 649 | Satzroute, auf die Aggregathuelle eingeschraenkt |
| Wachen erzwungen | 566 | Wachenroute, auf die Huelle der Default-Targets eingeschraenkt |
...
| Gleichung *Partition* | ✓ | 173 gegen 173 |
| Gleichung *Gate=Aggregat+1* | ✓ | 119 gegen 119 |
| Gleichung *Wachen* | ✓ | 576 gegen 576 |
| Gleichung *Satzroute* | ✓ | ok gegen ok |
|  |  |  |
| **LUECKEN (selbstzaehlend — Prosa zaehlt mit, mit Absicht)** |  |  |
| N1 roh | 146 | WORTvorkommen (\bsorry\b) ueber den verfolgten Bestand; zaehlt die eigene Dokumentation mit |
|   davon .lean | 103 | dieselbe Route, auf *.lean eingeschraenkt |
| Zeilen mit Vorkommen | 143 | ANDERE FRAGE als N1 (git grep -cw); nie als N1 lesen (§8 Fallstrick 9) |
|  |  |  |
...
|  |  |  |
| **DEFINITION-LEDGER** |  |  |
| Ledger-Zeilen | 95 | Zeilen-IDs in docs/definition-ledger.md |
| Referenzen im Bau | 81 | jedes #ledger_*-Kommando in DefinitionLedger.lean (auch #ledger_setzung) |
|  |  |  |
| **BAU (lake build)** |  |  |
| Build-Jobs | 1313 | lake build ueber die Default-Targets |
| geprueft (AxiomGate) | 3387 | Konstanten aus dem Importbaum, namensgefiltert auf Reformulation.* |
|  |  |  |
| **DOC-LINT** |  |  |
