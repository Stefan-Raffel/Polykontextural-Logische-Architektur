...
zu — oder hält fest, dass es keinen gibt. Sie ist Ledger Rev. 26.
...
**Alle Kennzahlen dieser Datei beziehen sich auf den Commit `cfe5a9e`.** Ein Bau-Zug macht
...
| L16-23 | Morphogramm (§16) | `KPC.card_rgs_eq_card_finpartition` | Theorem | Operationalisierung | ja, `[propext, Classical.choice, Quot.sound]` | die RGS-Reihen der Länge `n` und die Mengenpartitionen von `Fin n` sind **gleich viele** — die Kardinalitätsseite des Repräsentations-Theorems `K.rgs_equiv_partition`. Daneben ohne eigene Zeile — L05-7-Präzedenz: `KPC.card_finpartition_fin_two`, `KPC.card_finpartition_fin_three`, `KPC.card_finpartition_fin_four`, `KPC.card_finpartition_fin_five`, die vier Werte über die vorhandenen Marken der Berechnungs-Reihe. **Paragraphenwahl:** §16, weil der gezählte Gegenstand die Morphogramm-Normalform ist und die Stufenzahlen `KPS.card_rgs_two`, `KPS.card_rgs_three` und `KPS.card_rgs_five` unter L16-20 stehen; die Partition ist hier Zielseite und nicht Gegenstand. **Grenze:** die Bell-Zahl kommt nicht vor — dass `Nat.bell n` die Partitionen zählt, ist weder hier noch in Mathlib bewiesen, das die Aussage als offenen Posten führt. Und die Zahlen sind über `RGS` gerechnet, nicht über die Partitionen; der Satz transportiert sie |
...
| L21-7 | Drehrichtungswechsel (§21) | `NCat.drall_swap` | Theorem | Deutung | ja, `[propext]` | mit der anderen Negation begonnen ist die „einzig relevante Änderung … ein Vertausch der Drehrichtung“ (S. 37 [I12]): `K l` und `K r` tauschen, alles andere bleibt — für jede Station und jeden Abstand, bei drei Werten; die Einordnung (`NCat.kat`) ist unsere. Daneben `NCyc.tafelVI5_eq_reverse` (derselbe Hamiltonkreis rückwärts), `NCat.tabulierung1` und `NCat.tabulierung2` (die ART-Spalte beider Tabulierungen, wie gedruckt). Die dritte Spalte („U“ in jeder Zeile) ist nicht abgebildet (ruht, Architekt 25.9.2026). H1: Günthers „Zahlenwerte“ (S. 37 [I11]) sind STELLEN entlang des Weges, getrennt von der Inversionszahl min(k, 6 − k) (`NCat.zahlenwerte`) |
...
| L21-9 | Begriff und Zahl (§21) | `NCat.tabulierung1` | Theorem | Deutung | ja, `[propext]` | TEILGETRAGEN: die Zahlen für O und K bei drei Werten, auf dem alternierenden Weg (S. 37 [I11], [I15]) — Günthers „notwendig“ meint diese Zählung (Hermeneutes D1–D3, die Lesart „minimal“ ist gefallen). Daneben `NCat.tabulierung2` und `NCat.katalog_general`. „Jeden Begriff“ nicht. `NLen.inv_is_min_length` und `NCat.stationen_abstand` sind Bestandssätze OHNE Anker an S. 37 |
