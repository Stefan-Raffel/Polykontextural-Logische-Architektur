...
**Alle Kennzahlen dieser Datei beziehen sich auf den Commit `70f8a6e`.** Ein Bau-Zug macht
...
| L21-2 | Negation nur zwischen Nachbarn (§21) | `NCyc.sw` | Definition | Operationalisierung | keine (def) | ZUORDNUNG: die Bauform folgt der Regel — `sw i` tauscht nur die Werte `i` und `i + 1`; Günthers „darf“ (S. 25 [I2], [I3]) ist normativ, kein Satz trägt es. Daneben `NTC.three_iff_adj`, die verneinende Hälfte: entfernte Negatoren ergeben keinen Kreis (vgl. S. 48 [I21]) |
